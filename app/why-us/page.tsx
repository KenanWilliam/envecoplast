        ...
        {whyPanels.map((panel, index) => (
          <Reveal key={panel.title} delay={index * 0.04}>
            <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <panel.icon className="h-6 w-6 text-[#1A6B3C]" />
              <h2 className="mt-4 text-xl font-semibold text-slate-900">{panel.title}</h2>
              <p className="mt-2 text-sm leading-7 text-slate-600">{panel.body}</p>
            </article>
          </Reveal>
        ))}
      </section>
